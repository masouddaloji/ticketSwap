  ...
  return (
    <>
      ...
      {selected ? (
        <button
          key={selected.id}
          className="mb-6 w-full rounded-lg bg-[#f0fbfe] p-4 text-start"
        >
          <h4 className="flex items-center text-lg !text-foreground md:text-xl">
            {selected.name}
            <span className=" text-[#00b6f0]">
              <VerifiedIcon fill="currentColor" className="ml-1 h-4 w-4" />
            </span>
            <span
            ...
            >
              <CloseIcon
                fill="currentColor"
                className="  text-foregroundMuted transition-colors hover:text-foreground "
              />
            </span>
          </h4>
        ...
        <>
          <Form id="search-form" className="relative mb-6 flex" role="search">
            <span className="left absolute left-2 top-1/2 h-7 w-7  -translate-y-1/2">
              {searching ? (
            ...
            <input
              ...
              className="text-color h-14 flex-1 rounded-lg bg-elevatedBackground pl-12 text-lg focus-within:bg-white focus-within:shadow-[0_1px_2px_#1a21291a,0_4px_12px_#1a21291a] focus:outline-none"
              type="search"
              name="q"
          ...
          {searchResult.resp.data?.length ? (
            <section>
              ...
              {searchResult.resp.data?.map((event) => (
                <button
                  key={event.id}
                  className="mb-2 w-full rounded-lg bg-elevatedBackground p-4 text-start"
                  onClick={() => setSelected(event)}
                >
                  <h4 className=" text-color flex items-center text-lg text-foreground md:text-xl">
                    {event.name}
                    <span className=" text-[#00b6f0]">
                      <VerifiedIcon
                        fill="currentColor"
